...
# Backward-compatible SocksiPy class names used by older callers.
Socks5AuthError = SOCKS5AuthError
...
def wrap_module(module):
    ...
    Keep the historical guarded socket wrapper so non-INET/non-stream
    sockets are not proxied, and keep the create_connection() patch for
    SOCKS5/HTTP proxies to avoid local DNS leakage.
    """
    if socksocket.default_proxy:
        ...
        if socksocket.default_proxy[0] != PROXY_TYPE_SOCKS4:
...
def create_connection(dest_pair,
    ...
    **proxy_args - Same args passed to socksocket.set_proxy() if present.
    ...
    # when this function is installed as socket.create_connection(),
    # callers do not pass explicit proxy_* arguments; use the default proxy.
    use_default_proxy = proxy_type is None and socksocket.default_proxy
    if use_default_proxy:
        proxy_type, proxy_addr, proxy_port, proxy_rdns, proxy_username, proxy_password = socksocket.default_proxy
    elif proxy_type is None:
        return _orgcreateconnection(dest_pair, timeout, source_address)
    ...
    for r in socket.getaddrinfo(proxy_addr, proxy_port, 0, socket.SOCK_STREAM):
        ...
        try:
            ...
            if proxy_type and not use_default_proxy:
